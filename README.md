# 🎮 Steam 好友收割机 (Steam Friend Picker) v1.3.0
...
| 来源 | 抓什么 | 自动分组 |
|---|---|---|
| 小黑盒文章/讨论评论区 | 所有人贴的 Steam 链接 / 数字 ID / **好友代码（5~10 位短数字，自动换算）** / `[steam]xxx[/steam]` BBCode 签名档 | 小黑盒评论 |
...
- **暗色 / 浅色 双主题**（CSS 变量驱动）
- Toast 通知（成功 / 警告 / 错误三色）
- **跑批百分比进度条（v1.2.0）**：批量添加时顶栏出现绿色进度条，实时显示 `42%（5/12）`
- 响应进度条（运行中状态栏显示 `⚡ 5/30 ×1.5 · ✓3 友1 邀0 限1 败0`）

### 🔄 自动更新（v1.0.0 新增，v1.2.0 全程浏览器内完成）

> 「加载解压缩的扩展」天然没有自动更新（浏览器不允许扩展自重启），本工具的更新链路：

#### 🅰 浏览器内一键更新（v1.2.0 核心，无需任何桌面脚本）

- **更新横幅**：发现新版本时 popup 顶部出现橙色横条，点击展开 → 新版本号、发布日期、**更新内容摘要**（来自 release body）+「🚀 一键更新」按钮
- **一键更新页**：点按钮打开扩展内更新页（全屏），全自动三步带进度条：
  1. **下载** release zip（实时 KB / 百分比）
  2. **解压**（内置 zip 解析 + `DecompressionStream`，兼容 PowerShell 打包的反斜杠路径）
  3. **写入**扩展所在文件夹（`showDirectoryPicker` 授权一次，之后记住，全自动）
- **文件夹授权记忆**：首次选择扩展文件夹后存 IndexedDB，以后一键更新无需再选（浏览器重启后首次写入会自动请求补授权）
- **目标校验**：所选文件夹必须有 `manifest.json`，选错会提醒
- **完成后引导**：一键打开扩展管理页，点「重新加载」↻ 生效（浏览器安全机制，无法代劳的最后一步）
- 下载源优先 release asset（白名单打包），失败自动退回 codeload zipball（自动剥 `<repo>-<sha>/` 前缀）
...
> v1.1.x 及以前依赖桌面 `update.bat` + 桌面副本的方式已退役：浏览器加载的扩展文件夹可以是磁盘上任意位置，更新都在浏览器内完成，桌面不再需要任何脚本和副本。
...
弹窗「队列」Tab → **📂 导入** → 粘贴一段文本（每行一个，支持纯 17 位数字 / **好友代码（5~10 位短数字）** / 主页链接 / BBCode），扩展自动正则提取 + 后台解析自定义 URL。
...
| **自动驾驶** | **开（v1.3.0）** | 浏览器开着就自动按上述速率清空队列：后台每分钟检查一次，队列非空且没在跑就自动开跑一批；速率沿用间隔/自适应/时段/日配额约束，关闭浏览器即停 |
| 黄金时段 | 关 | 只在 9~23 点运行 |
| 完成桌面通知 | 开 | 跑完一波系统通知 |
